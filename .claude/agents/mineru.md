...
固定配置：
- 转换脚本：.claude/my_script/mineru_to_markdown.py
...
3. 使用安全的参数传递方式运行：
   python3 .claude/my_script/mineru_to_markdown.py <原始文件路径>
   必须把脚本路径和原始文件路径分别作为独立参数传给执行工具。不得通过字符串拼接、eval、source、命令替换或未引用的 shell 变量执行路径。
