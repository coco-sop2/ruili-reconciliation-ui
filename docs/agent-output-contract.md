...
```json
{"matched":true,"difference":0,"issues":"","period":"2026-08","name":"京东商城"}
```

存在差异：

```json
{"matched":false,"difference":-5,"issues":"ERP 销售额为 512042 元，结算单净营业额为 512047 元。","period":"2026-08","name":"京东商城"}
```

## 实现位置

- JSON 解析与严格校验：`server/src/lib/cherrystudio.ts` 的 `parseAgentResponse()`
- 结果写入数据库：`server/src/services/reconciliation.ts` 的 `applyReconciliationResult()`
